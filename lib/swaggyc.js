...
SwaggyC.prototype._commands = function (tasks, cb) {
  ...
  commands.forEach(function (command) {
    jobs.push(function (cb) {
      try {
        // merge parameters specified in command format in tasks.yml and CLI args
        // merged twice due to CLI args can contain parameters as well
        self.params.lang = command.meta.language;
        jazz.compile(command.format).process(self.params, function (command_result) {
          jazz.compile(command_result).process(self.params, function (result) {
            command.exec = result;
